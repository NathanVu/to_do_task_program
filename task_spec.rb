...
describe Task do
  ...
  it "it has to be able to show the status of a printed task that is done" do
    task = Task.new("title", "description")
    expect(task.print_status).to eq("Title: title / Description: description")
    expect(task.done).to eq("finished")
    expect(task.print_status).to eq("Title: title / Description: description / Status: finished")
  end
  # Story: As a developer, I can add all of my Tasks to a TaskList.
  # it "has to be able to be added to a task list" do
  #   task = Task.new("title", "description")
  #   expect(task.add_to_list(list))
  # end
end
